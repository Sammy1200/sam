...
-> `自动解析当前应进的大区`  
-> `启动器进游戏并进入交易行`  
-> `按启动器“上架”勾选决定是否执行预上架流程`
-> `判断账号是否还在冷却期，必要时等待`  
-> `进入抢购主循环`  
-> `过程中按规则做余额检测、轻量落库，并在上架勾选启用时执行 10 分钟循环上架`
-> `到达结束条件后最终写回 canonical`  
-> `按执行位规则切区或换号`  
-> `新账号按上架勾选决定是否先上架，再进入抢购`

临时抢购模式是旁路入口：

- 启动菜单 `[2]` 进入临时抢购模式
- 它复用主脚本中段链路：按上架勾选决定是否 `预上架`，再进入 `抢购循环`，并在上架勾选启用时执行定时上架
...
- 定向切换成功后，再接回正常“按上架勾选决定是否预上架 -> 抢购”链路
...
- 从交易行页面直接启动
- 不登录、不选区、不读账号库、不预上架
- 交易行正常时先沿用正常模式余额识别，再刷新进入道具详情页；详情页未命中 `meihuo.png` 时直接执行 `购买 -> 确定`
- 交易行命中 `jiaoyihang.png` 但未命中 `dianpu.png` 时按店铺异常走正常店铺恢复动作
- 余额不足、账号限制、未知异常、抢购上限等代码触发结束时会微信推送本机编号与本轮成功/失败汇总
- F12 参数窗已设置本次有效的 `抢购上限` 时，暴力模式无视余额不足，继续抢购到上限或其他结束条件
- 不加入换号链路，不写账号库、运行态快照、F12 快照或每日统计
- F12 暂停参数窗可设置本次内存有效的 `抢购上限`

饰品抢购模式是绑定正常账号库的独立入口：

- 从启动器选择 `饰品抢购` 进入
- 不走临时模式，读正常 canonical 账号上下文
- 全程不上架，不执行启动前预上架、周期上架或换号后预上架
- 换号/切区沿用正常链路，切换后进入饰品交易行继续饰品循环
- 饰品循环只使用饰品交易行模板、9 个饰品坐标和饰品颜色规则，不套用普通道具价格规则
- 运行写库使用独立饰品库 `C:\py666\accessory_account_stats.sqlite3`，不写正常石头库
- 饰品库沿用 canonical 表结构；`baseline_item_count` 在网页显示为饰品库存，`current_balance` 在网页显示为金币
- F12 暂停/恢复、轻量落库、抢购成功/失败、换号下号收尾沿用正常写库逻辑，但只写饰品库

装备抢购模式是绑定正常账号库的阶段性入口：

- 从启动器选择 `装备抢购` 进入
- 不走临时模式，读正常 canonical 账号上下文
- 全程不上架，不执行启动前预上架、周期上架或换号后预上架
- 换号/切区沿用正常链路，切换后跳过预上架并重新进入装备详情页执行筛选和循环抢购
- 装备价格识别只使用正常模式第二识别区域和同一套数字模板，不使用正常模式颜色守卫
- 装备循环命中价格后跳过道具点击，直接购买确认；不新增装备独立 SQLite
...
canonical SQLite 是唯一真实数据源，当前分为两个互相隔离的库：
...
查看页和公网页可通过 `石头库 / 饰品库` 切换展示；本机查看页切到哪个库就按同一套最小保存逻辑写哪个库，公网快照仍只开放查看与刷新。
