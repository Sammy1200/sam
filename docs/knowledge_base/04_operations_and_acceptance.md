...
- `暴力模式` 从交易行页面启动，不登录、不上架、不换号、不入库；交易行正常时先沿用正常模式识别余额，再刷新进入道具详情页，详情页未命中 `meihuo.png` 时直接执行 `购买 -> 确定`，不点击第一件商品；店铺异常走正常店铺恢复动作；余额不足、账号限制、未知异常、抢购上限等代码触发结束时会微信推送本机编号与本轮成功/失败汇总；若 F12 参数窗已设置本次有效的 `抢购上限`，余额不足不再结束暴力模式
