...
def get_number(frame, templates):
    try:
        ...
        if color_diff > 45: # [修正] 略微放宽容错至 45，防止因动态背景误跳过有货帧
            return None 
...
def get_balance(frame):
    ...
    try:
        ...
        if result and len(result) > 0:
            ...
            if res_str:
                # [修正] 核心 Bug：区分处理分隔符和单位小数点
                # 只有当识别到“万”或“亿”时，里面的符号才可能是小数点
                has_unit = any(u in res_str for u in ['万', '亿'])
                if has_unit:
                    res_str = res_str.replace("。", ".").replace(",", ".")
                else:
                    # 纯数字识别时，要把可能被错认成分隔符的符号直接剔除
...
def parse_balance(bal_str):
    ...
    try:
        ...
        if '亿' in bal_str:
            return int(round(num_val * 100000000)) # [修正] 使用 round 防止浮点数精度丢失
        elif '万' in bal_str:
...
def wait_and_recognize_balance(wait_time, camera):
    ...
    # [修正] 动态等待逻辑：先等 UI 出现，如果等到 1.4 秒还没出，也不再死等，直接在最后 0.1s 强行截一张
    ...
    try:
        while True:
            ...
            if target_stop_seconds > 0:
                curr_elapsed = int(total_running_time + (time.time() - last_resume_time)) if not IS_PAUSED else int(total_running_time)
